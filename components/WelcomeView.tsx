    ...
    <div className="flex flex-col md:flex-row min-h-[550px] font-arial">
      <div className="md:w-1/2 bg-slate-900 p-12 text-white flex flex-col justify-center relative overflow-hidden">
        ...
        <h1 className="text-5xl mb-6 leading-tight font-bold">
          ...
          <span className="text-pink-500 italic">{data.welcomeTitle.split(' ').slice(3).join(' ')}</span>
        </h1>
      ...
      <div className="md:w-1/2 p-12 flex flex-col justify-center items-center bg-white">
        <div className="w-24 h-24 bg-pink-50 rounded-3xl flex items-center justify-center mb-10 shadow-xl shadow-pink-50">
          <Shield className="text-pink-600" size={48} />
        </div>
        <h2 className="text-2xl font-bold text-slate-900 mb-4 tracking-tight uppercase">XÁC THỰC DANH TÍNH</h2>
        <p className="text-slate-500 text-center mb-12 max-w-xs leading-relaxed text-sm">
          Quý khách vui lòng nhấn nút bên dưới để xem nội dung hợp đồng và xác nhận chữ ký số.
        </p>
        <button
          onClick={onStart}
          className="w-full max-w-xs py-5 bg-pink-600 hover:bg-slate-900 text-white font-bold rounded-2xl shadow-2xl shadow-pink-100 transition-all active:scale-95 flex items-center justify-center space-x-4 group"
        >
